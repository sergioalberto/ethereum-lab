...
contract supplyChain {
    ...
    // A product object or group a product structure (collection of individual variables into one group)
    struct product {
        string modelNumber;
        string partNumber;
        string serialNumber;
        ...
        uint32 cost;
        uint32 mfgTimeStamp;  // Manufacturer timestamp
    }

    // Mapping one product / element to another (list of products using the product_id as key)
    ...
    struct ownership {
        uint32 productId;
        uint32 ownerId;
        uint32 trxTimeStamp; // transaction timestamp
        address productOwner;
    }
    ...
    function addProduct(uint32 _ownerId,
        ...
        // You could use a 'require()' too instead of 'if'
        if(keccak256(abi.encodePacked(participants[_ownerId].participantType)) == keccak256("Manufacturer")) { // compare hashes of the strings
